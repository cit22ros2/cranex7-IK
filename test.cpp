...
double IK(double L[], double xt[], double Rt[],double psi, double theta[]){
    ...
    double r[3][3];
    r[0][0] = cos(Rt[0])*cos(Rt[1])*cos(Rt[2]) + sin(Rt[0])*sin(Rt[2]);r[0][1] = -1*cos(Rt[0])*cos(Rt[1])*sin(Rt[2]) + -1*sin(Rt[0])*cos(Rt[2]);r[0][2] = cos(Rt[0])*sin(Rt[1]);
    r[1][0] = sin(Rt[0])*cos(Rt[1])*cos(Rt[2]) + cos(Rt[0])*sin(Rt[2]);r[1][1] = -1*sin(Rt[0])*cos(Rt[1])*sin(Rt[2]) + cos(Rt[0])*cos(Rt[2]);r[1][2] = sin(Rt[0])*sin(Rt[1]);
    r[2][0] = -1*sin(Rt[1])*cos(Rt[2]);r[2][1] = sin(Rt[1])*sin(Rt[2]);r[2][2] = cos(Rt[1]);
    double u_sw[3];
    ...
    theta20 = atan2( S2 , C2 );
    std::cout << theta10 << std::endl;
    std::cout << theta20 << std::endl;
    ...
    theta[0] = atan2( -1*( As[1][1]*sin(psi) + Bs[1][1]*cos(psi) + Cs[1][1] ), -1*( As[0][1]*sin(psi) + Bs[0][1]*cos(psi) + Cs[0][1] ) );
    theta[1] = acos( -1*( As[2][1]*sin(psi) + Bs[2][1]*cos(psi) + Cs[2][1] ) );
    theta[2] = atan2( As[2][2]*sin(psi) + Bs[2][2]*cos(psi) + Cs[2][2] , -1*(As[2][0]*sin(psi) + Bs[2][0]*cos(psi) + Cs[2][0] ) );
